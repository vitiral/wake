...
// #REQ-jsonnet
{
    ...
    module(name, inputs=null, outputs=null, exec=null, version=null, is_local=false):: {
        type: wake.TYPE_MODULE,
        name: name,
        inputs: inputs,
        outputs: outputs,

        assert exec != wake.EXEC_RESERVED,
        exec: exec,
        version: version,
        is_local: is_local,
    },
    ...
    retrievePkg(env, name, version, namespace):: {
        return: env.
    }

    example_file: wake.file("./example-file.txt"),
    example_dir: wake.dir("./example-dir/"),
    example_url: wake.module_url("foo.com", wake.hash("AES-256", "f3e230a")),
    example_module: wake.module(
        name='hello',
        inputs=[],
        outputs=[],
        exec=wake.exec('hello.wasm')
    ),
}
